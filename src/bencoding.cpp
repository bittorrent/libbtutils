...
void BencodedEmitterBase::Emit(const void *a, size_t len) {
	...
	{
		EmitChar('\"');
		const char* insertee = NULL;
		for (unsigned int i = 0; i < len; ++i) {
			int c = ((const char*)a)[i];
			switch (c) {
			case 8:
				insertee = "\\b";
				_emit_buf.insert(_emit_buf.end(), insertee, insertee + 2*sizeof(char));
				break;
			case 9:
				insertee = "\\t";
				_emit_buf.insert(_emit_buf.end(), insertee, insertee + 2*sizeof(char));
			...
			default:
				if (c < 32 || c > 127) {
					char s[7];
					//btsnprintf(s, sizeof(s)/sizeof(char), "\\u00%02x", c);
					snprintf(s, sizeof(s), "\\u00%02x", c);
					_emit_buf.insert(_emit_buf.end(), s, s + 6*sizeof(char));
				} else {
					EmitChar(c);
				}
			}
	...
	void EmitAsJson(const BencEntity *e, int indent = 0)
	{
		...
		case BENC_STR: {
			...
			EmitQuoted(me->GetRaw(), me->GetSize());
			break;
		}
		...
		case BENC_DICT: {
			...
			for(BencodedEntityMap::const_iterator it = ed->dict->begin(); it != ed->dict->end(); it++ ) {
				if (it != ed->dict->begin())
					Emit(", ", 2);
				EmitQuoted((const char*)it->first.GetRaw(), it->first.GetCount() );
				Emit(": ", 2);
				EmitAsJson(&it->second);
			}
			Emit(" }", 2);
			break;
		}
