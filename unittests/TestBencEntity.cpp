...
TEST(BencEntityClassTest, TestParseWithRegion)
{
	...
	for(int x=0; x<numTestStr; ++x){
		...
		BencEntity bEntity;
		BencEntity::Parse((const unsigned char *)&(bencMessage[0]), bEntity, (const unsigned char *)(&(bencMessage[0]) + bencMessage.length()), "r\0v\0", &region);
		...
		EXPECT_EQ(testText[x].size(), static_cast<unsigned int>(region.second - region.first));
...
TEST(BencEntityClassTest, TestParseInPlaceWithRegion)
{
	...
	for(int x=0; x<numTestStr; ++x){
		...
		// parse and get the region for 'v'
		BencEntity bEntity;
		BencEntity::ParseInPlace((const unsigned char *)&(bencMessage[0]), bEntity, (const unsigned char *)(&(bencMessage[0]) + bencMessage.length()), "r\0v\0", &region);
